...
export default function ReviewsTab() {
  ...
  async function handleToggle(row: Review) {
    ...
    try {
      const res = await fetch(`/api/admin/reviews?id=${row.id}`, {
        method: "PATCH",
        credentials: "include",
  ...
  async function handleSave() {
    ...
    try {
      if (modal === "add") {
        ...
        if (json.data) setData((prev) => [json.data, ...prev]);
      } else if (editId) {
        const res = await fetch(`/api/admin/reviews?id=${editId}`, {
          method: "PATCH",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        ...
        const json = await res.json();
        if (json.data) {
          setData((prev) => prev.map((r) => (r.id === editId ? json.data : r)));
        }
      }
      setModal(null);
    } catch {
    ...
    { key: "platform", label: "Platform" },
    {
      ...
      render: (row: Review) => (
        <button
          ...
          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors duration-300 focus:outline-none disabled:opacity-50 ${
            row.active ? "bg-green-500" : "bg-white/15"
          }`}
        >
          <span
  ...
  const pendingCount = data.filter((r) => !r.active).length;
...
function ReviewForm({
  ...
  form: Omit<Review, "id" | "created_at">;
  onChange: (f: Omit<Review, "id" | "created_at">) => void;
  onSave: () => void;
  saving: boolean;
}) {
