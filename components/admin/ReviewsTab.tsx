  ...
  const columns = [
    ...
    { key: "platform", label: "Platform" },
    {
      ...
      render: (row: AdminReview) => (
        ...
        >
          <span
            className={`inline-block h-3.5 w-3.5 rounded-full bg-white shadow transition-transform duration-300 ${row.active ? "translate-x-4" : "translate-x-1"}`}
          />
        </button>
      ),
    },
...
function ReviewForm({
  ...
  const set = (key: keyof ReviewFormPayload, val: unknown) =>
    ...
    <div className="flex flex-col gap-3 text-sm">
      ...
      <Field label="Account Bought">
        <input
          ...
          onChange={(e) => set("account_bought", e.target.value)}
          className={inp}
...
const inp =
  "bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder-white/25 focus:outline-none focus:border-indigo-500/60 transition-colors w-full";
const sel =
  "bg-[#1a1a1a] border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500/60 transition-colors w-full";
