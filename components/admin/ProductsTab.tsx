    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-white/40 mt-0.5">{data.length} listing{data.length !== 1 ? "s" : ""}</p>
        </div>
        <div className="flex gap-1.5">
      ...
      {modal && (
        <AdminModal title={modal === "add" ? "Add Product" : "Edit Product"} onClose={() => setModal(null)} size="lg">
...
function ProductForm({ form, onChange, onSave, saving }: {
  form: Omit<Product, "id">; onChange: (f: Omit<Product, "id">) => void;
  onSave: () => void; saving: boolean;
}) {
  const set = (k: keyof typeof form, v: unknown) => onChange({ ...form, [k]: v });
  const num = (k: keyof typeof form) => (e: React.ChangeEvent<HTMLInputElement>) => set(k, Number(e.target.value));
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        <Field label="Skins"><input type="number" value={form.skins} onChange={num("skins")} className={inp} /></Field>
        <Field label="Knives"><input type="number" value={form.knives} onChange={num("knives")} className={inp} /></Field>
        <Field label="Battle Passes"><input type="number" value={form.battle_passes} onChange={num("battle_passes")} className={inp} /></Field>
      </div>
      ...
      <div className="flex gap-5 pt-1">
        ...
        <Toggle label="Instant Delivery" checked={form.instant_delivery} onChange={(v) => set("instant_delivery", v)} color="emerald" />
      </div>
...
function Toggle({ label, checked, onChange, color = "white" }: {
  ...
  const dot = checked ? (color === "emerald" ? "bg-white translate-x-4" : "bg-[#0f0f0f] translate-x-4") : "bg-white translate-x-1";
  return (
