...
import { submitReview, resetSubmit } from "@/features/reviews/reviewsSlice";

const ACCOUNTS = [
  "Diamond Smurf — 25 Skins", "Platinum Starter — 12 Skins",
  "Immortal Beast — 40 Skins", "Gold Grinder — 8 Skins",
  "Ascendant Elite — 30 Skins", "Silver Fresh — 5 Skins",
  "Radiant Legend — 55 Skins", "Bronze Budget — 3 Skins",
];

export default function WriteReviewForm() {
  const dispatch = useAppDispatch();
  const { submitLoading, submitError, submitSuccess } = useAppSelector((s) => s.reviews);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    dispatch(submitReview({ ...form, rating }));
  }
  ...
  if (submitSuccess) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 bg-green-600/10 border border-green-500/30 rounded-2xl p-10 text-center">
        ...
        <button onClick={handleReset} className="text-sm text-red-400 hover:text-red-300 transition-colors mt-1">
    ...
    <form onSubmit={handleSubmit} className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col gap-4">
        ...
        <div className="flex gap-1">
          {[1,2,3,4,5].map((s) => (
            <button key={s} type="button"
              ...
              <Star size={24} className={s <= (hover || rating) ? "fill-yellow-400 text-yellow-400" : "text-white/20"} />
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          <input name="rank" value={form.rank} onChange={handleChange} placeholder="e.g. Diamond" className={inp} />
        </div>
        <div className="flex flex-col gap-1.5">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <textarea name="review" required rows={4} value={form.review} onChange={handleChange}
...
const inp = "bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none focus:border-red-500/50 transition-colors w-full";
const sel = "bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-red-500/50 transition-colors w-full";
