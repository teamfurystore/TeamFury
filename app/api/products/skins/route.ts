...
export async function GET(req: Request) {
  ...
  if (!parentProductId)
    return NextResponse.json({ success: false, error: "Missing parent_product_id" }, { status: 400 });
...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await supabase.from("product_items").insert(insertData).select();
    if (error) throw error;
    return NextResponse.json({ success: true, data });
  } catch (err) {
    console.error("POST /api/products/skins error:", err);
    return NextResponse.json({ success: false, error: "Failed to insert skins" }, { status: 500 });
  }
}

// ── PUT /api/products/skins ───────────────────────────────────────────────────
// REPLACE — atomically deletes all existing skins for a product then inserts
// the new set. This is the "Save" action from the admin SkinPickerPopup.
//
...
export async function PUT(req: Request) {
  try {
    ...
    return NextResponse.json({ success: false, error: "Failed to replace skins" }, { status: 500 });
  }
}

// ── DELETE /api/products/skins ────────────────────────────────────────────────
// Body: { parent_product_id }                     — delete ALL skins for a product
...
export async function DELETE(req: Request) {
  try {
    ...
    if (!id && !parent_product_id)
      return NextResponse.json({ success: false, error: "Missing id or parent_product_id" }, { status: 400 });
