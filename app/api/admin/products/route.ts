...
// Returns all products (active + inactive) with their associated skins
// (product_items) embedded so the admin table can show skin counts and
// the SkinPickerPopup can pre-populate with the already-saved selection.
//
// Response shape:
// [
//   {
//     id, slug, title, image, price, discounted_price, badge,
//     current_rank, peak_rank, skins, knives, battle_passes,
//     region, level, verified, instant_delivery, active,
//     description, profile_url, created_at,
//     product_items: [
//       { id, skin_id, display_name, display_icon, created_at }
//     ]
//   }
// ]
...
export async function GET(req: Request) {
  ...
  const [productsRes, skinsRes] = await Promise.all([
    ...
    db
      .from("product_items")
      .select("id, parent_product_id, skin_id, display_name, display_icon, created_at"),
  ]);

  if (productsRes.error)
    return NextResponse.json({ error: productsRes.error.message }, { status: 500 });
  if (skinsRes.error)
    return NextResponse.json({ error: skinsRes.error.message }, { status: 500 });
