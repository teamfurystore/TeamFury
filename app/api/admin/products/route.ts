...
export async function PUT(req: Request) {
  ...
  try {
    ...
    // ✅ Use the authenticated client tied to the user's token
    ...
    if (file) {
      ...
      // ✅ Use the same authenticated db client for storage too
      const { error: uploadError } = await db.storage
        .from("Thumbnails")
        .upload(fileName, buffer, { contentType: "image/webp" });
      if (uploadError) throw uploadError;

      const { data: urlData } = db.storage.from("Thumbnails").getPublicUrl(fileName);
      imageUrl = urlData.publicUrl;
    }
...
export async function GET(req: Request) {
  ...
  const [productsRes, skinsRes] = await Promise.all([
    db.from("products").select("*, active:is_active").order("created_at", { ascending: false }),
    db.from("product_items").select("id, parent_product_id, skin_id, display_name, display_icon, created_at"),
  ]);

  if (productsRes.error) return NextResponse.json({ error: productsRes.error.message }, { status: 500 });
  if (skinsRes.error) return NextResponse.json({ error: skinsRes.error.message }, { status: 500 });
...
const getFilePathFromUrl = (url: string): string | null => {
  try {
    const match =
      url.match(/\/storage\/v1\/object\/public\/Thumbnails\/(.+)$/) ?? url.match(/\/object\/public\/Thumbnails\/(.+)$/);
...
export async function DELETE(req: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ success: false, error: "Missing id in query" }, { status: 400 });
    }

    // 🔹 1. Fetch product image
    const { data: product, error: fetchError } = await db.from("products").select("image").eq("id", id).single();
    ...
    if (imageUrl) {
      ...
      if (filePath) {
        ...
        console.warn("⚠️ Unable to extract storage path from image URL:", imageUrl);
      }
    }
